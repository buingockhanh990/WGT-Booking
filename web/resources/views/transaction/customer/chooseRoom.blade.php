...
@endsection
@section('content')
    @include('transaction.customer.progressbar')
                ...
                <div class="card shadow-sm border">
                    <div class="card-body p-3">
                        ...
                        <p>{{ request()->input('count_person') }}
                            {{ Helper::plural('People', request()->input('count_person')) }} <on></on>
                            {{ Helper::dateFormat(request()->input('check_in')) }} to
                            {{ Helper::dateFormat(request()->input('check_out')) }}</p>
                        <hr>
                        <form method="GET"
                            action="{{ route('customer.chooseRoom', ['customer' => $customer->id]) }}">
                            <div class="row mb-2">
                                ...
                                <div class="col-lg-12">
                                    <div
                                        ...
                                        <div class="col p-4 d-flex flex-column position-static">
                                            ...
                                            <a href="{{ route('customer.confirmation', ['customer' => $customer->id, 'room' => $room->id, 'from' => request()->input('check_in'), 'to' => request()->input('check_out')]) }}"
